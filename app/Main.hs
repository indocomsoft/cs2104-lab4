...
printSolution query table = traverse printVar (variables query) >> return ()
  where printVar v@(Var x) = putStrLn $ x ++ " = " ++ show (resolve table v)

interpret :: Program -> Rel -> [Subs]
interpret prog rel = startSearch prog rel
...
run_prog program = do
  ...
  case parseRel command of
    Right rel -> do
      ...
      (traverse (printSolution rel) answers >> showTruth (length answers))
